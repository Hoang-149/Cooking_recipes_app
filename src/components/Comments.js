...
const Comments = ({
  ...
  const onDelReplyCommentPress = (id_user_cm, id_comment, id_replay) => {
    Alert.alert('Bạn có chắc?', 'Bạn có chắc muốn xóa bình luận này không?', [
      // The "Yes" button
      {
        text: 'Yes',
        onPress: () => {
          if (id_user_cm === user?.id || userCuisine === user?.id) {
            ...
            onReplyCommentDeleted(id_replay, id_comment);
          } else {
            ToastAndroid.show(
              'Bạn không thể xóa bình luận',
              ToastAndroid.SHORT,
        ...
        <View
          ...
          {user ? (
            <View
              ...
              <View style={{flexDirection: 'row'}}>
                {reply.id_user === user?.id || userCuisine.id === user?.id ? (
                  <TouchableOpacity
                    onPress={() =>
                      onDelReplyCommentPress(
                        reply.id_user,
                        reply.id,
                        comment.id,
                      )
